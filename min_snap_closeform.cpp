...
void minsnapCloseform::calQ() {
  ...
  for (int seg_index = 0; seg_index < n_seg; ++seg_index) {
    Eigen::MatrixXd Q_j(Eigen::MatrixXd::Zero(n_per_seg, n_per_seg));
    for (int i = 5; i <= n_order + 1; i++) {
      for (int j = 5; j <= n_order + 1; j++) {
        Q_j(i - 1, j - 1) = fact(i) / fact(i - 4) * fact(j) / fact(j - 4) /
                            (i + j - 7) * pow(ts(seg_index), i + j - 7);
      }
    }
    Q.block(seg_index * n_per_seg, seg_index * n_per_seg, n_per_seg,
            n_per_seg) = Q_j;
  }
}
...
void minsnapCloseform::calM() {
  ...
  for (int k = 0; k < n_seg; k++) {
    ...
    M.block(k * (n_order + 1), k * (n_order + 1), n_order + 1, n_order + 1) =
        M_k;
  }
}
...
void minsnapCloseform::calCt() {
  ...
  Eigen::MatrixXd dF_Ct =
      Eigen::MatrixXd::Zero(n_seg * (n_order + 1), 8 + (n_seg - 1));
  Eigen::MatrixXd dP_Ct =
      Eigen::MatrixXd::Zero(n_seg * (n_order + 1), (n_seg - 1) * 3);
...
minsnapCloseform::MinSnapCloseFormServer(const Eigen::VectorXd &wp) {
  ...
  R_pp = R.block(n_seg + 7, n_seg + 7, (n_seg - 1) * 3, (n_seg - 1) * 3);
  R_fp = R.block(0, n_seg + 7, n_seg + 7, (n_seg - 1) * 3);
...
Eigen::VectorXd minsnapCloseform::getTime() { return ts; }
